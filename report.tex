...
Under our initial plan we wanted to keep things as simple as possible and went with a purely sensor based approach where the current readings can be read by the computer in order to tell the user the current state of the bike and the user would update the settings and whatever it is that is being changed would change. This is very easy and cheap to implement but not really very suitable for the following reasons:
\begin{itemize}
...
\end{itemize}
This is what led us to our final cloud based approach. All of the settings will be stored in a database and when a user logs into a bike they will be able to download their settings and have the bike set up in the way that they like. 


\subsection{User Access}
In order to properly implement the bike settings we would need each user to have their own login
