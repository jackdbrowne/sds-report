...
\paragraph{}This section explains the considerations that were involved for each critical component on the product and the iterations until a practical solution was found. The practical solutions and the iterations that lead to the final solution is in section \textbf{\ref{sec:component_solutions} \nameref{sec:component_solutions}} on page \textbf{\pageref{sec:component_solutions}}. 
...
\subsection{Methods}
\paragraph{} After reviewing a few research papers, we came to the conclusion that our system needed to undergo 3 steps to monitor nearby obstacles (as mentioned in \citep{1300862}): motion detection, vehicle tracking and activity understanding.
...
\begin{algorithm}[h]
\caption{Computer Board to microcontroller brake motor control}\label{lag:comp_board_motor_control}
\begin{algorithmic}[1]
\Function{CalculateMotorControl}{$brakeData$}\Comment{Digital braking data received from the microcontroller}
...
\begin{algorithm}[h]
\caption{Brake Lever}\label{alg:brake_lever}
\begin{algorithmic}[1]
\Function{Brake}{$pin$}\Comment{The pin that received the analog signal}
...
\While{$ADCRegisterGOBit == $}\Comment{Wait for conversion to complete}
   ...
   \If{$pin == LeftPin$}\Comment{Which Brake lever was pressed}
   	\State $Motor := LeftMotor$
  \ElsIf{$pin == RightPin$}
  	\State $motor := RightMotor$\Comment{store right motor pin data in $motor$ variable}
\EndIf
\State $brakeLoad := \Call{CalculateBrakeForce}{adcResult}$
\State \Call{ApplyBrake}{$motor, brakeLoad$}
\EndFunction
\end{algorithmic}
\end{algorithm}
